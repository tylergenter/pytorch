...
void* ObjectPtrAllocator::malloc(int64_t size) {
  return allocator->malloc(allocatorContext, size);
}


void* ObjectPtrAllocator::realloc(void* ptr, int64_t size) {
...
#ifdef WITH_NUMPY
void* NumpyArrayAllocator::realloc(void* ptr, int64_t size) {
...
template<typename T>
static void * malloc_wrapper(void *ctx, int64_t size) {
  return ((T*)ctx)->malloc(size);
}

template<typename T>
static void * realloc_wrapper(void *ctx, void *ptr, int64_t size) {
